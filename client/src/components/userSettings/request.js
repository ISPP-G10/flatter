import { requestInputs } from "../../forms/requestForm";
...
const Request = () => {
    ...
    function handleRequestFormSubmit({values}){
        ...
        let result = `curl -X POST -F 'version=1.3' -F 'auth_user=inquilino_basico_rest' -F 'auth_pwd=iTopInquilinoBasico_1' http://itop.socialflatter.es/webservices/rest.php -F 'json_data={"operation":"core/create","class":"UserRequest","fields":{"title":"${values.title}","description":"${values.description + "    REPORTADO POR: " + localStorage.getItem("user")}","org_id":"SELECT Organization AS O WHERE O.id = 5"},"comment":"Prueba de uso API","output_fields":"id"}'`
        client.mutate({
            mutation: usersAPI.createRequest,
            variables: {
                command: result,
            }
        }).then((response) => {
            alert("Sugerencia registrada correctamente");
    ...
    return(
        <>
            <h2 className='section-title'>Sugerir cambios</h2>
            <FlatterForm
                ref={requestFormRef}
